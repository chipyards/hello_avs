...
typedef struct winampVisModule {
  ...
  HINSTANCE hDllInstance; // instance handle to this DLL (filled in by winamp)
  int sRate;             // sample rate (filled in by winamp)
  ...
  int spectrumNch;                      // filled by plugin
  int waveformNch;
...
typedef struct {
  ...
  const char *description; // description of library
